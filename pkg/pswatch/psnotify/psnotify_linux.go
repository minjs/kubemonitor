...
const (
	...
	PROC_EVENT_ALL = PROC_EVENT_FORK | PROC_EVENT_EXEC | PROC_EVENT_UID | PROC_EVENT_GID | PROC_EVENT_EXIT
)
...
func (w *Watcher) handleEvent(data []byte) {
	...
	case PROC_EVENT_FORK:
		...
		if w.isWatching(ppid, PROC_EVENT_EXEC) {
			// follow forks
			if watch, ok := w.watches[ppid]; ok {
				w.Watch(pid, watch.flags)
			} else if watch0, ok0 := w.watches[0]; ok0 {
				w.Watch(pid, watch0.flags)
			}
